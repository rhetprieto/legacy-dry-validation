# -*- coding: utf-8 -*-
RSpec.describe LegacyDry::Validation do
  ...
  let(:base_class) do
    Class.new(LegacyDry::Validation::Schema) do
      def self.messages
        LegacyDry::Validation::Messages.default.merge(
  ...
  describe 'defining schema with custom predicates container' do
    ...
    context 'when configured globally' do
      subject(:schema) do
        LegacyDry::Validation.Schema(base_class) do
          required(:email) { filled? & email? }
        end
      end

      before do
        LegacyDry::Validation::Schema.predicates(Test::Predicates)
      end
  ...
  it 'raises an error when message is missing' do
    ...
    expect { schema.(email: 'foo').messages }.to raise_error(
      LegacyDry::Validation::MissingMessageError, /email/
    )
  end
